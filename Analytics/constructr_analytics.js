    ...
    function constructr_analytics(URI,TIMESTAMP)
        {
            ...
            var IS_UUID = readCookie('constructr_analytics')

            if(!IS_UUID)
            {
                  var NEW_UUID = UUID();
                  createCookie('constructr_analytics',NEW_UUID,365);
            }

            UUID = readCookie('constructr_analytics')
            var REF = document.referrer;
            var B = navigator.appName;
            var B_NICK = navigator.appCodeName;
            var B_VERSION = navigator.appVersion;
            var B_HTTP_STRING = navigator.userAgent;
            var B_PLATTFORM = navigator.platform;
            var B_LANGUAGE = navigator.language;
            var S_PIXELDEPTH = screen.pixelDepth;
            var S_COLORDEPTH = screen.colorDepth;
            var S_AVAIL_HEIGHT = screen.availHeight;
            var S_AVAIL_WIDTH = screen.availWidth;
            var S_HEIGHT = screen.height;
            var S_WIDTH = screen.width;
            var END_RESULT = ('###CONSTRUCTR_ANALYTICS###' + URI + ':::' + TIMESTAMP + ':::' + UUID + ':::' + B + ':::' + B_NICK + ':::' + B_VERSION + ':::' + B_HTTP_STRING + ':::' + B_PLATTFORM + ':::' + B_LANGUAGE + ':::' + S_PIXELDEPTH + ':::' + S_COLORDEPTH + ':::' + S_AVAIL_HEIGHT + ':::' + S_AVAIL_WIDTH + ':::' + S_HEIGHT + ':::' + S_WIDTH + ':::' + REF);                

            reqwest
            (
                {
                    url:'http://constructr.phaziz.com/constructr/analytics/',
                    method:'post',
